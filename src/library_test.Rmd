...
# [3.2.1] licks and events for 4 days of FR baseline

-   Library merges data with metadata, adding useful columns to make analysis pipeline easier
-   Proposal to add experimental group + small label to identify experiment

```{r}
# adding experimental groups
test_data_merge <- test_data_merge %>% 
  mutate(
    group = case_when(
      ID %in% c(320, 323, 325, 326) ~ "uncertainty",
      TRUE ~ "control"
    )
  )

# Get total licks per spout for each session
activity_per_session <- test_data_merge %>% 
  group_by(
    ID,
    n_sesion,
    tipo_recompensa,
...
licks_p <- activity_per_session %>% 
  ggplot(aes(
    n_sesion,
    licks,
    color = tipo_recompensa,
    group = tipo_recompensa
  )) +
  geom_point() +
  geom_line() +
  facet_wrap(group~ID, ncol = 4, scales = "free") +
  # AESTHETICS
  theme_bw() +
  labs(color = "Reward type") +
  ylab("Number of licks per spout") +
  xlab("FR5 session number") +
  theme(
    text = element_text(size = 14)
  ) +
  scale_colour_manual(
    values = c("agua" = "blue", "sacarosa" = "#006400")
    )

event_p <- activity_per_session %>% 
  ggplot(aes(
    n_sesion,
    events,
    color = tipo_recompensa,
  ...
  theme_bw() +
  labs(color = "Reward type") +
  ylab("Number of events per spout") +
  xlab("FR5 session number") +
  theme(
    text = element_text(size = 14)
  ) +
...
# [3.2.2] licks and events for 4 days of FR baseline by group

```{r}
# same plot as above by group + point estimate error
activity_per_session_group <- activity_per_session %>%
  group_by(
    group,
    n_sesion,
    tipo_recompensa
  ...
  summarise(
    licks_m = mean(licks),
    events_m = mean(events),
    err_l = sd(licks) / sqrt(n()),
    err_e = sd(events) / sqrt(n())
  ) %>% 
  ungroup()

licks_g_p <- activity_per_session_group %>% 
  ...
  ylab("Number of licks sucrose spout") +
  xlab("FR5 session number") +
  theme(
    text = element_text(size = 14)
  ) +
  scale_colour_manual(
    values = c("control" = "blue", "uncertainty" = "#006400")
    )

events_g_p <- activity_per_session_group %>% 
  filter(tipo_recompensa == "sacarosa") %>% 
  ggplot(aes(
    n_sesion,
    events_m,
  ...
  theme_bw() +
  labs(color = "Experimental group") +
  ylab("Number of events sucrose spout") +
  xlab("FR5 session number") +
  theme(
    text = element_text(size = 14)
  ) +
...
# corrected data
corrected <- time_activity_corrected_data %>% 
...
corrected %>% 
  bind_cols(
    session_length_uncorrected = uncorrected$session_length
  )
```
...
This show typical ILI structure with licks mode around 120~ ms and failed licks around 250~ ms
...
interval_estimate_data %>% 
  ...
  geom_vline(xintercept = 125, color = "gray10") +
  geom_vline(xintercept = 260, color = "gray10") +
  theme_bw() +
  labs(color = "Experimental groupo") +
  ylab("Frequency") +
  xlab("Interlick Interval") +
  theme(
    text = element_text(size = 14),
...
bin_calculation <- function(merged_data, bin_size_ms){
  ...
  tmp <- out %>% drop_na()
  bin_n <- max(tmp$bins) + 1
  return(
    out %>% 
      mutate(bins = replace_na(bins, bin_n))
  )
}
```
...
bin_calculation_data %>% 
  ...
  summarise(
    licks_per_bin_m = mean(licks_per_bin),
    err = sd(licks_per_bin) / sqrt(n())
  ) %>% 
  ggplot(aes(
    bins,
    licks_per_bin_m,
    color = group,
    ymin = licks_per_bin_m - err,
    ymax = licks_per_bin_m + err
  )) +
  geom_point() +
  geom_line() +
  geom_errorbar() +
...
burst_estimates calculates clusters of licks based on a fixed threshold typical = 250 - 1000 ms
...
this function count clusters poor optimization
